...
class GeistGetter(BaseGetter):
    ...
    def _start(self):
        ...
        self.token_addresses.append(self.pool_info[ContractConst.coin_base_address].lower())
    ...
    def get_reward_requests(self, list_rpc_call, list_call_id, wallet_address, block_number='latest', **kwargs):
        add_rpc_call(
            abi=self.pool_info[ContractConst.multi_fee_abi], contract_address=self.pool_info[ContractConst.multi_fee_address],
            fn_name="earnedBalances", block_number=block_number, fn_paras=wallet_address,
            list_call_id=list_call_id, list_rpc_call=list_rpc_call
        )
    ...
    def decode_reward_info(self, decoded_response, wallet_address, block_number='latest', *args, **kwargs):
        ...
        reward_id = f'earnedBalances_{self.pool_info[ContractConst.multi_fee_address]}_{wallet_address}_{block_number}'.lower()
        reward_amount = decoded_response[reward_id][0] / 10 ** 18

        token_price = token_prices.get(self.pool_info[ContractConst.coin_base_address].lower()) or 0
